...
def calibration_recommendation(records, configured_target, reference_width=0.0):
    """Return a robust Target recommendation from real release records.

    Each sample is normalized back to one stable meter width. The candidate
    corrects the configured Target by the observed release error; a shot that
    actually released 2 px early recommends a Target 2 normalized px higher.
    Median/MAD filtering prevents one bad contour or reacquisition frame from
    steering the session.
    """
    usable = []
    widths = []
    for rec in records:
        if not isinstance(rec, dict) or rec.get("fire_mode") == "screen_cue":
            continue
        fill = _to_float(rec.get("fill_px"), -1.0)
        target = _to_float(rec.get("target_px"), -1.0)
        width = _to_float(rec.get("meter_width_px"), -1.0)
        if fill <= 0.0 or target <= 0.0 or width < 4.0:
            continue
        usable.append((fill, target, width))
        widths.append(width)
    if not usable:
        return None

    ref_width = _to_float(reference_width, 0.0)
    if ref_width < 4.0:
        ref_width = statistics.median(widths)
    base_target = _to_float(configured_target, 46.0)
    candidates = [
        base_target + (target - fill) * (ref_width / width)
        for fill, target, width in usable
    ]
    center = statistics.median(candidates)
    deviations = [abs(value - center) for value in candidates]
    mad = statistics.median(deviations)
    limit = max(1.5, 4.4478 * mad)  # 3 robust standard deviations.
    filtered = [value for value in candidates if abs(value - center) <= limit]
    if not filtered:
        filtered = candidates
    recommendation = statistics.median(filtered)
    spread = statistics.median([abs(value - recommendation) for value in filtered])
    return {
        "target": round(max(0.0, min(600.0, recommendation)), 1),
        "reference_width": round(ref_width, 1),
        "used": len(filtered),
...
class LicenseDialog(QDialog):
    ...
    def activate_key(self):
        ...
        if result.get("ok"):
            ...
            if parent is not None and hasattr(parent, "_refresh_license_badge"):
                parent._refresh_license_badge()
            self.activate.setText("ACTIVATED")
...
class MainWindow(QMainWindow):
    ...
    def _refresh_license_badge(self):
        """Reload license state and update the badge immediately."""
        self._last_license_check = time.monotonic()
        try:
        ...
        if self._unlocked:
            remaining = format_remaining(
                self._license_info.get("expires_in"), compact=True)
            self.license_badge.setText(
                remaining if remaining == "LIFETIME" else f"{remaining} LEFT")
        else:
